...
FairChange's AgentCore Runtime version 3 is protected at the service boundary by an Amazon Cognito custom JWT authorizer in `eu-north-1`. The authorizer uses the Cognito discovery document for the `fairchange-prod-users` pool and allows the `fairchange-public` app client. The public client identifier is not a secret; no client secret or AWS credential is stored in this repository.
...
- A bounded free-form prompt invocation returned HTTP 200 on the audited runtime.
- The runtime reported `READY` on version 3 with `customJWTAuthorizer` configured and the Cognito discovery URL.
